...
def running(mode: str, name: str | None) -> None:
    ...
    if mode == "prod":
        ...
        click.secho(
            f"INFO     :: Deploy on {'http'  if not app.config[CONFIG_LOCATION['app_config']].use_https else 'https'}://0.0.0.0:{str(80 if not app.config[CONFIG_LOCATION['app_config']].use_https else 443)}",
            fg="green",
        )
    elif mode == "test":
        app.prepare(
        ...
        click.secho("INFO     :: Deploy on http://0.0.0.0:80", fg="green",)
    else:
        ...
        click.secho("INFO     :: Deploy on http://127.0.0.1:6969", fg="green",)
